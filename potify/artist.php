...
//Strip urlencoded characters such as "+" for spaces
$artist = urldecode( $_GET[ "artist" ] );

//Fetch one track for use in displaying the artists name in the header
$statement = $connection->prepare( "SELECT * FROM tracks WHERE artist = ? GROUP BY artist" );
$statement->bind_param( "s", $artist );
$statement->execute();

$result = $statement->get_result();
$artistTrackData = $result->fetch_assoc();
///////////////////////////////////////////

//Fetch all track data for specified artist for use in displaying tracks
$statement = $connection->prepare( "SELECT * FROM tracks WHERE artist = ?" );
$statement->bind_param( "s", $artist );
$statement->execute();
...
<div class="container-fluid text-left bg-primary text-white" style="padding-top: 10vh; padding-bottom: 10vh">
    ...
    <div class="container mx-auto w-1200">
        ...
        <h1 class="display-2 my-4"><?php echo( $artistTrackData[ "artist" ] ); ?></h1>
        <p>All tracks</p>
...
<div class="container-fluid text-center ">
    ...
    <div class="container w-1200">
        ...
        <!-- Table to hold all tracks by specified artist -->
        <table class="table table-hover text-left">
            ...
            //Loop through all the tracks and display them along with their average rating
            while ( $albumTracks = $result->fetch_assoc() ) {

                //Fetch average review of current track
